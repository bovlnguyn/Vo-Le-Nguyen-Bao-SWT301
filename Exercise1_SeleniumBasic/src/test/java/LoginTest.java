...
class LoginTest {
    ...
    @CsvFileSource(
            resources = "/login-data.csv",
            numLinesToSkip = 1,
            emptyValue = "",                 // ô rỗng -> ""
            nullValues = {"NULL", "null"}    // các chữ này -> null
    )
    ...
    void testLoginWithCSV(String username, String password, String expectedResult) {
        ...
        // --- Normalize input từ CSV ---
        username = username == null ? "" : username.strip();          // hoặc .trim() nếu JDK 8
        password = password == null ? "" : password.strip();
        expectedResult = expectedResult == null ? "" : expectedResult.strip().toLowerCase();
        ...
        // --- Xác định locator theo expectedResult đã chuẩn hoá ---
        By messageLocator = "success".equals(expectedResult)
                ? By.cssSelector(".flash.success")
                : By.cssSelector(".flash.error");
        ...
        // --- Assert nội dung thông báo (khử khoảng trắng/thường hoá chữ) ---
        String msg = message.getText().strip().toLowerCase();
